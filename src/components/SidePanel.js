...
=======
  ...
  const onSubmitCard = (event) => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_BACKEND_URL}/boards/${props.selectedBoard}/cards`,
        {
          message: cardFields.message,
        }
      )
  ...
  const onDeleteBoard = () => {
    axios
      .delete(
        `${process.env.REACT_APP_BACKEND_URL}/boards/${props.selectedBoard}`
      )
      .then((response) => {
        console.log(response);
        // ask Michelle to include board_id in success response message
  ...
  return (
    <div className="sidepanel-main">
      <h3>Select Board to Display:</h3>
      <select id="board-list" onChange={props.onSelectBoard}></select>
...
>>>>>>> 4411fae199b3c86600ac389e04389702885b9c62
};
